...
old1 = """      const { data: { session } } = await supabase.auth.getSession()
      if (!session) { navigate('/'); return }"""

new1 = """      const { data: { session } } = await supabase.auth.getSession()
      if (!session) { navigate('/'); return }
      setMeuUserId(session.user.id)"""

old_state = """  const [irmaoExpandido, setIrmaoExpandido] = useState(null)"""
new_state = """  const [irmaoExpandido, setIrmaoExpandido] = useState(null)
...
if old_state not in content:
    print("ERRO: bloco state nao encontrado")
    erro = True
