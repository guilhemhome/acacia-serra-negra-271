...
export default function Login() {
  ...
  const cadastroConcluido = location.state?.cadastroConcluido
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const { data: assocData, error: assocErr } = await supabase
        .from('associados').select('id, email, user_id').eq('cpf', cpfLimpo).maybeSingle()
      ...
      const { data: p, error: perfilErr } = await supabase.from('perfis_acesso').select('perfil').eq('user_id', uid).single()
          ...
          <div className="p-6">
            ...
            {cadastroConcluido && (
              <div className="bg-green-50 border border-green-200 text-green-700 rounded-xl px-4 py-3 mb-4 text-sm">
                ✅ Acesso criado com sucesso! Entre com seu CPF e senha para completar seu perfil.
              </div>
            )}
