...
export default function Dashboard() {
  ...
  async function buscarDados() {
    ...
    const anivMes = (aniversarios || []).filter(a => {
      ...
      return new Date(a.data_nascimento).getMonth() + 1 === mesAtual
    }).map(a => ({
      nome: a.nome_completo,
      dia: new Date(a.data_nascimento).getDate()
...
export default function Dashboard() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'linear-gradient(135deg,#1a237e 0%,#283593 50%,#1565c0 100%)', padding:'24px 16px', fontFamily:'sans-serif' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'24px' }}>
        ...
        <button onClick={sair} style={{ marginLeft:'auto', background:'rgba(255,255,255,0.15)', border:'none', borderRadius:'8px', width:'36px', height:'36px', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }} title="Sair">
              ...
              <div style={{ padding:'16px' }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
                  {stats.aniversarios.map((a, i) => (
                    <div key={i} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px', background:'#f9fafb', borderRadius:'8px' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                        <div style={{ width:'32px', height:'32px', borderRadius:'50%', background:'#dbeafe', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'11px', fontWeight:'600', color:'#1d4ed8' }}>
          ...
          <div style={{ background:'white', borderRadius:'16px', overflow:'hidden' }}>
            ...
            <div style={{ padding:'16px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px' }}>
                {[
                  { icon:'👥', label:'Aprovações', bg:'#eff6ff', onClick: () => navigate('/aprovacoes') },
                  { icon:'➕', label:'Novo cadastro', bg:'#f0fdf4', onClick: () => navigate('/cadastro') },
                  { icon:'📋', label:'Ver membros', bg:'#fefce8', onClick: () => {} },
                  { icon:'⚙️', label:'Configurações', bg:'#fdf4ff', onClick: () => {} },
                ].map(b => (
                  <div key={b.label} onClick={b.onClick} style={{ padding:'12px', border:'0.5px solid #e5e7eb', borderRadius:'10px', display:'flex', alignItems:'center', gap:'8px', cursor:'pointer', background:'white' }}>
