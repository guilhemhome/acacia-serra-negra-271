...
export default function Dashboard() {
  ...
  async function buscarTudo() {
    ...
    // Verificar cache
    try {
      const { data: { user: userCache } } = await supabase.auth.getUser()
      const CACHE_KEY = 'dashboard_cache_' + (userCache?.id || 'anon')
      const raw = sessionStorage.getItem(CACHE_KEY)
      if (raw) {
        const { ts, dados } = JSON.parse(raw)
        if (false) { // cache desabilitado
    ...
    const [
      ...
      supabase.from('perfis_acesso').select('perfil, is_admin').eq('user_id', user.id).maybeSingle(),
      ...
      supabase.from('mensagens_templates').select('chave, mensagem').in('chave', ['aniversario_irmao_whatsapp', 'aniversario_dependente_whatsapp']).then(r => ({ data: r.data || [], error: null })).catch(() => ({ data: [], error: null })),
    ...
    const apelido = assoc?.apelido || nome.split(' ')[0]
    ...
    const tObj = {}
    ;(tmpl || []).forEach(t => { tObj[t.chave] = t.mensagem })
    setTemplates(tObj)
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#1a237e 0%,#283593 50%,#1565c0 100%)', padding: '24px 16px 40px' }}>
      <div style={{ maxWidth: 480, margin: '0 auto' }}>
        ...
        {proximoEvento && resumoPresencas && (() => {
          const podeVerModal = ['ADM', 'Venerável Mestre', 'Chanceler', 'Secretário'].includes(usuario.perfil)
          return (
          <div style={{ background:'rgba(255,255,255,0.95)', borderRadius:16, padding:'14px 16px', marginBottom:20 }}>
            ...
            <div style={{ display:'flex', gap:8 }}>
              <div onClick={() => podeVerModal && setModalPresencas('confirmados')} style={{ flex:1, textAlign:'center', background:'#e8f5e9', borderRadius:10, padding:'10px 4px', cursor: podeVerModal ? 'pointer' : 'default' }}>
              ...
              <div onClick={() => podeVerModal && setModalPresencas('ausentes')} style={{ flex:1, textAlign:'center', background:'#ffebee', borderRadius:10, padding:'10px 4px', cursor: podeVerModal ? 'pointer' : 'default' }}>
              ...
              <div onClick={() => podeVerModal && setModalPresencas('pendentes')} style={{ flex:1, textAlign:'center', background:'#f1f5f9', borderRadius:10, padding:'10px 4px', cursor: podeVerModal ? 'pointer' : 'default' }}>
        ...
        {/* Modal presenças */}
        {modalPresencas && (
          <div onClick={() => setModalPresencas(null)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
            <div onClick={e => e.stopPropagation()} style={{ background:'#fff', borderRadius:16, padding:20, width:'100%', maxWidth:400, maxHeight:'80vh', overflowY:'auto' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
                <h3 style={{ margin:0, color:'#1a237e', fontSize:15 }}>
                  {modalPresencas === 'confirmados' && '✅ Confirmados'}
                  {modalPresencas === 'ausentes' && '❌ Ausentes'}
                  {modalPresencas === 'pendentes' && '⏳ Pendentes'}
                </h3>
                <button onClick={() => setModalPresencas(null)} style={{ background:'none', border:'none', fontSize:20, cursor:'pointer', color:'#64748b' }}>×</button>
              </div>
              {modalPresencas === 'confirmados' && (
                listaPresencas.confirmados.length === 0
                  ? <p style={{ color:'#64748b', fontSize:13 }}>Nenhum confirmado.</p>
                  : listaPresencas.confirmados.map((p, i) => (
                    <div key={i} style={{ padding:'8px 0', borderBottom:'1px solid #f1f5f9', fontSize:14, color:'#1e293b' }}>👤 {p.nome}</div>
                  ))
              )}
              {modalPresencas === 'ausentes' && (
                listaPresencas.ausentes.length === 0
                  ? <p style={{ color:'#64748b', fontSize:13 }}>Nenhuma ausência registrada.</p>
                  : listaPresencas.ausentes.map((p, i) => (
                    <div key={i} style={{ padding:'8px 0', borderBottom:'1px solid #f1f5f9' }}>
                      <div style={{ fontSize:14, color:'#1e293b', fontWeight:500 }}>👤 {p.nome}</div>
                      {p.motivo && <div style={{ fontSize:12, color:'#64748b', marginTop:2 }}>Motivo: {p.motivo}</div>}
                    </div>
                  ))
              )}
        ...
        {podeVerFinanceiro && (
          <div style={{ background:'rgba(255,255,255,0.95)', borderRadius:16, padding:'14px 16px', marginBottom:20 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
              <p style={{ margin:0, fontSize:12, fontWeight:700, color:'#1a237e', textTransform:'uppercase', letterSpacing:'0.05em' }}>💰 Situação Financeira</p>
              <span style={{ fontSize:10, background:'#f1f5f9', color:'#64748b', borderRadius:20, padding:'2px 8px', fontWeight:600 }}>Em breve</span>
            </div>
            <div style={{ display:'flex', gap:8 }}>
              <div style={{ flex:1, textAlign:'center', background:'#e8f5e9', borderRadius:10, padding:'10px 4px' }}>
                <div style={{ fontSize:22, fontWeight:800, color:'#2e7d32' }}>—</div>
                <div style={{ fontSize:11, color:'#2e7d32' }}>Quites</div>
              </div>
              <div style={{ flex:1, textAlign:'center', background:'#ffebee', borderRadius:10, padding:'10px 4px' }}>
                <div style={{ fontSize:22, fontWeight:800, color:'#c62828' }}>—</div>
                <div style={{ fontSize:11, color:'#c62828' }}>Pendentes</div>
              </div>
              <div style={{ flex:1, textAlign:'center', background:'#fff8e1', borderRadius:10, padding:'10px 4px' }}>
                <div style={{ fontSize:22, fontWeight:800, color:'#f59e0b' }}>—</div>
                <div style={{ fontSize:11, color:'#f59e0b' }}>Em atraso</div>
              </div>
            </div>
            <p style={{ margin:'10px 0 0', fontSize:11, color:'#94a3b8', textAlign:'center' }}>Módulo financeiro em desenvolvimento</p>
          </div>
        )}
