...
old1 = """  const [perfisEditaveis, setPerfisEditaveis] = useState(['Venerável Mestre','Secretário','Financeiro','Administrativo','Membro','Ritualística','Hospitalaria'])
  useEffect(() => {
    async function carregar() {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session) { navigate('/'); return }"""

new1 = """  const [perfisEditaveis, setPerfisEditaveis] = useState(['Venerável Mestre','Secretário','Financeiro','Administrativo','Membro','Ritualística','Hospitalaria'])
...
new2 = """  async function toggleAdmin(userId, atual, nome) {
    if (userId === meuUserId) { msg('Por segurança, você não pode conceder/revogar ADM da sua própria conta por aqui.'); return }
    const novoValor = !atual
    const aviso = novoValor
      ? `Conceder acesso ADM (total) para "${nome}"?\\n\\nEssa pessoa passará a ter acesso irrestrito a todo o sistema.`
      : `Revogar acesso ADM de "${nome}"?\\n\\nEssa pessoa perderá acesso administrativo total.`
